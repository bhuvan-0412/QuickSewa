...
export default function Dashboard() {
  ...
  if (!authed) {
    return (
      <main style={{
        ...
        background: 'linear-gradient(135deg, #f0fdf4 0%, #f8fafc 100%)', padding: '2rem 1rem',
        position: 'relative'
      }}>
        <div style={{
          position: 'absolute',
        ...
        <div className="animated-card" style={{
          ...
          border: '1px solid var(--primary-border)', textAlign: 'center',
          ...
          <h1 style={{ fontSize: 24, fontWeight: 800, color: '#14532d', marginBottom: 6, letterSpacing: '-0.5px' }}>
            {t.officerLogin}
          </h1>
          ...
          <button
            ...
            style={{
              ...
              minHeight: '52px', cursor: 'pointer', border: 'none',
  ...
  return (
    <main style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f0fdf4 0%, #f8fafc 100%)', paddingBottom: '5rem' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Link href="/" style={{
            ...
            fontSize: 16, color: '#374151', textDecoration: 'none',
            display: 'flex', alignItems: 'center', justifyContent: 'center'
          }}>←</Link>
          <div>
            <h1 style={{ fontSize: 18, fontWeight: 800, color: '#14532d', margin: 0, letterSpacing: '-0.5px' }}>
              {t.dashboardTitle}
            </h1>
            <p style={{ fontSize: 12, color: '#6b7280', margin: 0, fontWeight: 500 }}>{t.dashboardSub}</p>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
      ...
      <div style={{
        ...
        gap: 16, padding: '2rem 1.5rem 1.25rem'
      }}>
        {[
          ...
          <div key={s.label} className="hover-lift" style={{
            ...
            padding: '1.25rem 1rem', border: '1px solid #e2e8f0', textAlign: 'center',
            boxShadow: 'var(--shadow-sm)'
          }}>
            ...
            <p style={{ fontSize: 12, color: '#6b7280', marginTop: 4, margin: '4px 0 0 0', fontWeight: 600 }}>{s.label}</p>
          </div>
        ))}
      </div>

      {/* Dashboard filter selector buttons bar */}
      <div style={{ padding: '0 1.5rem', marginBottom: '1.5rem', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        {['all', 'open', 'in-progress', 'resolved'].map(s => (
          <button
            key={s}
            onClick={() => setFilterStatus(s)}
            style={{
              padding: '0 1.1rem', borderRadius: 99, fontSize: 13, fontWeight: 700,
      ...
      <div style={{ padding: '0 1.5rem', display: 'flex', flexDirection: 'column', gap: 16 }}>
        {error ? (
          <div style={{
            ...
            borderRadius: 16, padding: '1.25rem', color: '#dc2626',
            textAlign: 'center', fontSize: 14, fontWeight: 700
          }}>
            ⚠️ {error}
          </div>
        ) : loading ? (
          <p style={{ textAlign: 'center', color: '#9ca3af', padding: '3rem', fontWeight: 600 }}>{t.loading}</p>
        ) : filtered.length === 0 ? (
          <p style={{ textAlign: 'center', color: '#9ca3af', padding: '3rem', fontWeight: 600 }}>{t.noComplaints}</p>
        ) : (
          filtered.map(c => {
            const sla = slaLabel(c.created_at, c.status)
            return (
              <div key={c.id} className="animated-card hover-lift" style={{
                background: 'white', borderRadius: 20,
                ...
                <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 0 }}>
                  ...
                  <div style={{ padding: '1.25rem', flex: 1, minWidth: 260 }}>
                    ...
                    {c.title && (
                      <h3 style={{ fontSize: 16, fontWeight: 800, color: '#111827', margin: '4px 0 6px 0', letterSpacing: '-0.3px' }}>
                        {c.title}
                      </h3>
                    )}
                ...
                <div style={{
                  ...
                  {c.status !== 'in-progress' && c.status !== 'resolved' && (
                    <button
                      ...
                      style={{
                        padding: '0.5rem 1.25rem', borderRadius: 10, fontSize: 13, fontWeight: 700,
                  ...
                  {c.status === 'resolved' && (
                    <span style={{ fontSize: 13, color: '#16a34a', fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                      {t.issueResolved}
                    </span>
                  )}
                </div>
